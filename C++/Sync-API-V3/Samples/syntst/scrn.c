                              ...
                              //           TYPE_LONG).
{
   ...
   #error "Kein Zielsystem definiert"
      ...
      #ifndef S_BOOT
         if (bHex == TRUE)         /* Eingabe erfolgte in Hex => konvertieren */
            sscanf(&pszInput[2], "%lx", &ulResult);
         else
            sscanf(pszInput, "%lu", &ulResult);
      #else
         if (bHex == TRUE)         /* Eingabe erfolgte in Hex => konvertieren */
...
/*
@func
This function converts binary data into a hexadecimal string. <nl>
The length of <p pszHexString> is exactly two times <p ulDataSize> plus
one ('\\0'); the memory has to be allocated before the function
is called. <nl>
@xref MEM_HexStringToBin
*/



OKERR ENTRY MEM_BinToHexString (
             GPBYTE pbData,      //@parm in | Pointer on binary data.
             UINT  uiDataSize,   //@parm in | Size of data.
             GPCHAR pszHexString,//@parm out | Pointer on hexadecimal string.
             UINT  uiStringSize) //@parm in | Size of string.
{
   register GPBYTE  pbItem;
   register GPCHAR  pszPos;
   register INT     i;
   CHAR             c;
   ...
   */
   if (uiStringSize < uiDataSize * 2)
      {
      return (OKERR_BUFFER_OVERFLOW);
      }

   pbItem = pbData;
   pszPos = pszHexString;
   for (i = 0; i < (INT)uiDataSize; i++)
      {
      c = (CHAR) (HI_NIB(*pbItem));
      *pszPos++ = (CHAR) (HEXDIGIT (c));
      c = (CHAR) (LO_NIB(*pbItem));
      *pszPos++ = (CHAR) (HEXDIGIT (c));
      pbItem++;
      }
   *pszPos = '\0';
