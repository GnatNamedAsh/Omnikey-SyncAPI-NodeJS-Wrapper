...
{
	strcpy_s(szReaderName,pszReaderName);
}
...
int CCardReport::ShowData(int iType,unsigned char* pucData)
{
	...
	if(iType==0x04)
	{
		...
		for(i=0;i<8;i++)
			sprintf_s(szText,"%s%02X ",szText,pucData[i]);
...
void CCardReport::OnDetail() 
{
	...
	if(rc != SUCCESS)
	{
		...
		if(rc != SUCCESS)
		{
			sprintf_s(szText,"Unable to select page number %d",ucCurPageNumber);
...
OnDetailSkipLoadKey:
	...
	case 0: 
		...
		case 0x1F:
			...
			{
				sprintf_s(szText,"Invalid Application Limit Byte. 0x%02X",ucConfigBlock[0]);
