...
BOOL CCheckProtectedDlg::OnInitDialog()
{
	...
	LONG ret=SCardEstablishContext(SCARD_SCOPE_USER,NULL,NULL,&hContext_temp);


	if(SCARD_S_SUCCESS==ret)//(SCardEstablishContext(SCARD_SCOPE_USER,NULL,NULL,hContext)))
	{
		
		// SCardEstablishContext successfull
		if(SCARD_S_SUCCESS==(SCardListReaders(hContext_temp,NULL,mszReaders,&dwLength)))
		{
				...
				for(int j=0;j<32;j++)
				{
					...
					else
					{
						...
						else
						{
							char bufferx[50];
							char msg[100];
							strcpy_s(msg,"Error occured\nErrorcode: 0x");
							// convert errorcode from int to a string
							_itoa_s(errorcode,bufferx,16);
							strcat_s(msg,bufferx);
...
{
	if(SCARD_S_SUCCESS==(SCardConnect(hContext_temp,selreader,SCARD_SHARE_SHARED,SCARD_PROTOCOL_T0|SCARD_PROTOCOL_T1,&hCard,&pdwActiveProtocol)))
	{
